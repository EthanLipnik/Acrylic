...
struct GenerateMeshIntent: AppIntent {
    ...
    @Parameter(title: "Grain Alpha", default: 0.5, controlStyle: .slider, inclusiveRange: (0.0, 1.0))
    var grainAlpha: Double
    
    @Parameter(title: "Subdivisions", default: 18, controlStyle: .stepper, inclusiveRange: (2, 128))
    var subdivisions: Int
    
    @Parameter(title: "Location Randomizer Range", default: 0.2, controlStyle: .slider, inclusiveRange: (0.0, 0.8))
    var locationRandomizeRange: Double
    
    @Parameter(title: "Turbulency Randomizer Range", default: 0.25, controlStyle: .slider, inclusiveRange: (0.0, 0.8))
    ...
    func perform() async throws -> some IntentResult {
        let mesh = MeshKit.generate(
            ...
            locationRandomizationRange: -Float(locationRandomizeRange)...Float(locationRandomizeRange),
            turbulencyRandomizationRange: -Float(turbulencyRandomizeRange)...Float(turbulencyRandomizeRange)
        )
        
        let url = try await mesh.export(size: MeshSize(width: width, height: height), 
