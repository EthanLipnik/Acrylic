...
class ProjectNavigatorViewController: UIViewController, UICollectionViewDelegate, UICollectionViewDragDelegate {
    ...
    override func viewDidLoad() {
        ...
        newProjectButton.menu = UIMenu(title: "New Project", image: UIImage(systemName: "add"), children: [
            UIAction(title: "Mesh Gradient", handler: { _ in
            ...
            UIAction(title: "3D Scene", state: .off, handler: { _ in
                Task(priority: .userInitiated) { [weak self] in
                    do {
        ...
        _ = directoryWatcher?.startWatching()
    }
    ...
    override func viewDidDisappear(_ animated: Bool) {
        ...
        _ = directoryWatcher?.stopWatching()
    }
    ...
    func collectionView(_ collectionView: UICollectionView, contextMenuConfigurationForItemAt indexPath: IndexPath, point: CGPoint) -> UIContextMenuConfiguration? {
        ...
        return .init(identifier: nil, previewProvider: nil) { [weak self] _ in
            var children: [UIMenuElement] = [
                UIMenu(title: "", options: .displayInline, children: [
                    UIAction(title: "Delete", image: UIImage(systemName: "trash"), discoverabilityTitle: "Delete document", attributes: .destructive, handler: { _ in
            ...
            if document.fileUrl?.pathExtension != "icloud" {
                ...
                if UIDevice.current.userInterfaceIdiom != .phone {
                    children.insert(UIAction(title: "Open in New Window" + (fileUrls.count > 1 ? " (\(fileUrls.count))" : ""), discoverabilityTitle: "Open document(s) in new window", handler: { _ in
                        fileUrls.forEach({ self?.view.window?.openDocument($0, destroysCurrentScene: false, alwaysUseNewWindow: true) })
                    }), at: 0)
                }

                if fileUrls.count == 1 {
                    children.insert(UIAction(title: "Open", discoverabilityTitle: "Open document", handler: { _ in
                        self?.view.window?.openDocument(fileUrl)
                    }), at: 0)

                    if AppDelegate.isCloudFolder {
                        children.insert(UIAction(title: "Remove download", discoverabilityTitle: "Remove download of document", handler: { _ in
                            do {
                ...
                if fileUrls.count == 1 {
                    children.insert(UIAction(title: "Download", discoverabilityTitle: "Download document", handler: { _ in
                        do {
